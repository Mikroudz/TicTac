...
src = cv2.imread('testikuvat/current4.jpg')
...
warp_tresh = cv2.adaptiveThreshold(warp_gray,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV,101,1)
warp_laplacian = cv2.Laplacian(warp_tresh,cv2.CV_8U)

warp_absd = cv2.convertScaleAbs(warp_laplacian)
warp_edged = cv2.Canny(warp_gray, 30,150, apertureSize = 3)
...
for i in range(3):
    for j in range(3):
        x.append(warp_edged[int(i*maxHeight/3):int((i+1)*maxHeight/3),int(j*maxWidth/3):int((j+1)*maxWidth/3)].copy())

for i in range(9):
    cnts = cv2.findContours(x[i], cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    cnts = imutils.grab_contours(cnts)
    cnts = sorted(cnts, key = cv2.contourArea, reverse = True)[:10]
    shape = []
    for c in cnts:
    ...
    #circles = cv2.HoughCircles(x[i].copy(),cv2.HOUGH_GRADIENT, 1.2, 10)
    #if circles is not None:
    #	circles = np.round(circles[0, :]).astype("int")

    #	for (x, y, r) in circles:
    #		cv2.circle(warp_edged, (x+i % 3 * (maxWidth // 3), y+i // 3 * (maxHeight // 3)), r, (0, 255, 0), 4)
